...
export class EntitiesClient {
    ...
    generateUmlDiagram(id: string, templateId: string): Observable<FileResponse> {
        let url_ = this.baseUrl + "/api/Entities/{id}/generate/{templateId}/uml";
        if (id === undefined || id === null)
            throw new Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id));
        if (templateId === undefined || templateId === null)
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGenerateUmlDiagram(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGenerateUmlDiagram(<any>response_);
                } catch (e) {
    ...
    protected processGenerateUmlDiagram(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
